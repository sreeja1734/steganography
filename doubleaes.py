...
def double_aes_encrypt(message, key1, key2):
    # First encryption
...
def double_aes_decrypt(ciphertext, key1, key2):
    # First decryption
    first_decrypted = decrypt(ciphertext, key2)
    
    # Second decryption
    original_message = decrypt(first_decrypted, key1)
...
def encode(file, message, salt):
    ...
    even_ciphered_data = double_aes_encrypt(even_message, keyeven, key2) + b'[END]'
    odd_ciphered_data = b'[DNE]'+ double_aes_encrypt(odd_message, keyodd, key2)
...
def decode(file, salt):
    ...
    ev_message = double_aes_decrypt(evmsg_bytes[:-5], keyeven, key2)
    ...
    od_message = double_aes_decrypt(odmsg_bytes[5:], keyodd, key2)
...
key2=b'\x9c\x0eE?\xa8\r\x0b\xae\xe58U\xb8\x18\xbe\xbbEl\x9c\xba\xab{K"\x89\xde\xdbi8\x91;\xa4\x99'
datas = []
